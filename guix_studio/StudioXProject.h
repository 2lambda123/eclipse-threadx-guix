...
// schema changes, so that we can add code to handle previous versions
// correctly.
...
/* This enabled checking if the project was created by a newer version of
   Studio. Turn this off for testing when the Studio version number is not
   accurate
*/
...
enum HEADER_IDS { THEME_HEADER = 4096 };
...
enum GROUP_IDS { COLOR_GROUP = 4096, FONT_GROUP, PIXELMAP_GROUP, STRING_GROUP };
...
enum DECODER_TYPE { DECODER_TYPE_NONE = 0, DECODER_TYPE_SW, DECODER_TYPE_HW };
...
enum ID_TYPES { ID_TYPE_WIDGET = 0, ID_TYPE_ANIMATION };
...
class palette_info {
public:
  palette_info() { memset(this, 0, sizeof(palette_info)); }
...
//
// If you modify the members of this class in any way, make SURE you initialize
// them in the default constructor, and even more important make SURE you copy
// the new members in the copy constructur!!
/***************************************************************************************/
class res_info {
public:
  res_info(int ResType = 0);
  res_info(const res_info *in_parent, const res_info &other,
           BOOL copy_next); // copy constructor
...
public:
  ...
  widget_info(const widget_info &other,
              BOOL copy_next = FALSE); // copy constructor
  ...
  CString base_name;   // GUIX widget type ("GX_WINDOW"), or template name
                       // ("BASE_SCREEN")
  CString custom_name; // not currently used, the idea is allow user to specify
                       // custom control block type
  ...
  BOOL visible_at_startup; // Indicates if the widget is visible when the
...
public:
  ...
  folder_info(const folder_info &other,
              BOOL copy_next = FALSE); // copy constructor
...
public:
  ...
  void GenerateResourceXml(CString &pathname, CString &filename,
                           res_info *start);
  ...
  INT GetIdIndex(const CArray<id_info> &dictionary,
                 const CString &id_name) const;
  CString GetIdName(int DisplayIndex, int id_type, int index) const;
  void CleanupIdDictionary(int DisplayIndex, int id_type);
  BOOL CopyIdDictionary(int Display, int id_type,
                        CArray<id_info> *copied_dictionary);
  void SortIdDictionary(CArray<id_info> *dictionary);

  // resource dictionary
  BOOL AddToResourceDictionary(int DisplayIndex, res_info *info,
                               int res_id = -1);
  BOOL RemoveFromResourceDictionary(int DisplayIndex, res_info *info);
  BOOL UpdateDictionaryResourceName(int DisplayIndex, CString &old_name,
                                    res_info *info);
  GX_RESOURCE_ID GetResourceId(int DisplayIndex, const res_info *info) const;
  GX_RESOURCE_ID GetResourceId(int DisplayIndex, const int restpe,
                               const CString &name) const;
  BOOL GetResourceName(int DisplayIndex, int res_type, int index,
                       CString &return_name) const;
  BOOL CopyDictionary(int DisplayIndex, int res_type,
                      CArray<CString> *dictionary);
  void SortResDictionary(INT res_type, CArray<CString> *dictionary);

  res_info *FindResource(int DisplayIndex, int ThemeIndex, int res_type,
                         GX_RESOURCE_ID res_id) const;
  res_info *FindResource(int Display, int theme, int restype,
                         const CString &name) const;
  res_info *FindResource(const res_info *start, int restype,
                         const CString &name) const;
  res_info *FindResourceFolder(int Display, int theme_id, int res_type,
                               GX_RESOURCE_ID res_id,
                               const CString &name) const;
  res_info *FindResourceFolder(const res_info *start, int restype,
                               int folder_id, CString name = _T("")) const;

  widget_info *FindWidgetInfo(const GX_WIDGET *widget) const;
  widget_info *FindWidgetInfo(const folder_info *folder,
                              const GX_WIDGET *widget) const;
  widget_info *FindWidgetInfo(const widget_info *start,
                              const GX_WIDGET *widget) const;
  widget_info *FindParentInfo(const widget_info *start,
                              const widget_info *child) const;
  widget_info *FindParentInfo(const folder_info *folder,
                              const widget_info *child) const;
  widget_info *FindParentInfo(const widget_info *child) const;
  widget_info *FindWidgetInfo(const widget_info *start, const CString &name,
                              BOOL search_child) const;
  widget_info *FindWidgetInfo(const folder_info *folder, const CString &name,
                              BOOL search_child) const;
  ...
  folder_info *FindFolderInfo(const folder_info *start,
                              const CString &name) const;
  ...
  void WriteOneResource(xml_writer &writer, res_info *res,
                        GX_BOOL xml_mode = FALSE);

  BOOL IsWidgetInInfoTree(const widget_info *start,
                          const widget_info *info) const;
  ...
  BOOL InitializeOnePixelmap(res_info *info, palette_info *palette,
                             int display_color_format = -1);
...
protected:
  ...
  void WriteThemePaletteInfo(xml_writer &writer, theme_info *theme,
                             BOOL xml_mode = FALSE);
  ...
  void WriteResources(xml_writer &writer, res_info *start,
                      GX_BOOL xml_mode = FALSE);
  ...
  void ReadResources(xml_reader &reader, int display, int theme,
                     res_info *parent);
  void ReadThemeScrollbars(xml_reader &reader, int display_index, int theme);
  void ReadThemePaletteInfo(xml_reader &reader, theme_info *theme,
                            BOOL xml_mode = FALSE);
