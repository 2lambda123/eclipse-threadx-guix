...
/*                                                                        */
/**************************************************************************/
static UINT _gx_single_line_text_input_pen_down_process(
  ...
  if (start_mark != end_mark) {
    ...
    _gx_single_line_text_input_text_rectangle_get(
        text_input, (INT)(start_mark - end_mark), &dirty_area);
  ...
  _gx_single_line_text_input_position_get(
      text_input,
      (UINT)((*event_ptr).gx_event_payload.gx_event_pointdata.gx_point_x));

  /* Set highlight start/end mark to insert position. */
  text_input->gx_single_line_text_input_start_mark =
      text_input->gx_single_line_text_input_insert_pos;
  text_input->gx_single_line_text_input_end_mark =
      text_input->gx_single_line_text_input_insert_pos;
...
/*                                                                        */
/**************************************************************************/
static UINT _gx_single_line_text_input_pen_drag_process(
    GX_SINGLE_LINE_TEXT_INPUT *text_input, GX_EVENT *event_ptr) {
  GX_RECTANGLE dirty_area;
  UINT start_mark = text_input->gx_single_line_text_input_start_mark;
  UINT end_mark = text_input->gx_single_line_text_input_end_mark;
  ...
  if (text_input->gx_widget_status & GX_STATUS_OWNS_INPUT) {
    ...
    if ((click_x < client.gx_rectangle_left) &&
        (text_input->gx_single_line_text_input_end_mark > 0)) {
      if (!(text_input->gx_widget_status &
            (GX_STATUS_MARK_NEXT | GX_STATUS_MARK_PREVIOUS))) {
      ...
      text_input->gx_widget_status |= GX_STATUS_MARK_PREVIOUS;
               ...
               (text_input->gx_single_line_text_input_end_mark <
                text_input->gx_single_line_text_input_string_size)) {
      if (!(text_input->gx_widget_status &
            (GX_STATUS_MARK_NEXT | GX_STATUS_MARK_PREVIOUS))) {
    ...
    if ((start_mark != text_input->gx_single_line_text_input_end_mark) ||
      ...
      _gx_single_line_text_input_text_rectangle_get(
          text_input,
          (INT)(end_mark - text_input->gx_single_line_text_input_end_mark),
          &dirty_area);
...
UINT _gx_single_line_text_input_event_process(
  ...
  case GX_EVENT_SHOW:
    ...
    if ((text_input->gx_widget_style & GX_STYLE_CURSOR_ALWAYS) &&
      ...
      _gx_system_timer_start((GX_WIDGET *)text_input, ID_TEXT_INPUT_TIMER,
  ...
  case GX_EVENT_STYLE_CHANGED:
    if (widget->gx_widget_status & GX_STATUS_VISIBLE) {
      ...
      if ((old_style & (GX_STYLE_BORDER_MASK | GX_STYLE_TEXT_ALIGNMENT_MASK)) !=
          (widget->gx_widget_style &
           (GX_STYLE_BORDER_MASK | GX_STYLE_TEXT_ALIGNMENT_MASK))) {
        _gx_single_line_text_input_position_update(text_input);
      }
    }
    break;
  ...
  case GX_EVENT_FOCUS_GAINED:
    if (!(text_input->gx_widget_style & GX_STYLE_CURSOR_ALWAYS)) {
      text_input->gx_widget_status |=
          (GX_STATUS_CURSOR_SHOW | GX_STATUS_CURSOR_DRAW);

      if (text_input->gx_widget_style & GX_STYLE_CURSOR_BLINK) {
        blink_interval = text_input->gx_single_line_text_input_cursor_instance
                             .gx_text_input_cursor_blink_interval;
        _gx_system_timer_start(widget, ID_TEXT_INPUT_TIMER,
                               (UINT)blink_interval, (UINT)blink_interval);
      }
    }

    /* Select all text. */
    _gx_single_line_text_input_text_select(
        text_input, 0, text_input->gx_single_line_text_input_string_size - 1);
  ...
  case GX_EVENT_TIMER:
    if (event_ptr->gx_event_payload.gx_event_timer_id == GX_MARK_TIMER) {
      ...
      _gx_text_input_cursor_dirty_rectangle_get(
          &text_input->gx_single_line_text_input_cursor_instance, &dirty_area);
  ...
  case GX_EVENT_FOCUS_LOST:
    if (!(text_input->gx_widget_style & GX_STYLE_CURSOR_ALWAYS)) {
      ...
      /* If cursor always is not set, do-not receive resize notify when lose
       * focus */
      text_input->gx_widget_status &= ~GX_STATUS_RESIZE_NOTIFY;
    }
    ...
    if (text_input->gx_single_line_text_input_start_mark !=
      ...
      _gx_single_line_text_input_text_rectangle_get(
          text_input,
          (INT)(text_input->gx_single_line_text_input_start_mark -
                text_input->gx_single_line_text_input_end_mark),
          &dirty_area);
