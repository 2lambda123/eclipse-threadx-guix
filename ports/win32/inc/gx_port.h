...
#define GX_WIN32_EVENT_THREAD_CREATE(driver_data, thread_name)                 \
  driver_data->win32_driver_thread_handle =                                    \
      CreateThread(NULL, GX_WIN32_STACK_SIZE,                                  \
                   (LPTHREAD_START_ROUTINE)gx_win32_driver_thread_entry,       \
                   driver_data, 0, 0);
...
#define GX_THREADS_ONLY_CALLER_CHECKING                                        \
  if ((_tx_thread_system_state) || (_tx_thread_current_ptr == TX_NULL) ||      \
...
CHAR _gx_version_id[] = "Copyright (c) Microsoft Corporation. All rights "
                        "reserved.  *  GUIX Win32/Visual Version 6.4.0 *";
#else
