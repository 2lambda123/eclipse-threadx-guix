...
VOID _gx_dave2d_simulation_display_driver_rotated_glyph_4bit_draw(
  ...
  pixel_width =
      (UINT)(draw_area->gx_rectangle_bottom - draw_area->gx_rectangle_top + 1);
  ...
  y_height =
      (UINT)(draw_area->gx_rectangle_right - draw_area->gx_rectangle_left + 1);
  ...
  GX_SET_BLEND_FUNCTION(
      blend_func, context->gx_draw_context_display->gx_display_color_format)
...
#endif
        blend_func(context, (INT)((UINT)(rotated_left) + col),
                   (INT)((UINT)(rotated_top) + row), text_color, alpha);
      }
      col++;
      glyph_data++;
    }
