...
VOID _gx_dave2d_simulation_display_driver_32bpp_glyph_1bit_draw(
  ...
  pixel_per_row = (GX_VALUE)(draw_area->gx_rectangle_right -
  ...
  /* Compute the number of useful bytes from the glyph this routine is going to
     use. Because of map_offset, the first byte may contain pixel bits we don't
     need to draw; And the width of the draw_area may produce part of the last
     byte in the row to be ignored. */
  num_bytes = (UINT)((map_offset->gx_point_x) + pixel_per_row + 7) >> 3;
  /* Take into account if map_offset specifies the number of bytes to ignore
  ...
  /* Compute the number of pixels to draw from the last byte, if there are more
   * than one byte in a row. */
  if (num_bytes != 1) {
    pixel_in_last_byte =
        (map_offset->gx_point_x + (GX_VALUE)pixel_per_row) & 0x7;
