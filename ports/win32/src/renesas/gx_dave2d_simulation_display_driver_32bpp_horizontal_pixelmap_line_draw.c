...
_gx_dave2d_simulation_display_driver_32bpp_horizontal_pixelmap_line_compressed_write(
  ...
  if ((write_func == GX_NULL) || (blend_func == GX_NULL)) {
    return;
  }
...
VOID _gx_dave2d_simulation_display_driver_32bpp_horizontal_pixelmap_line_draw(
  ...
  if (info->pixelmap->gx_pixelmap_flags & GX_PIXELMAP_COMPRESSED) {
    ...
    _gx_dave2d_simulation_display_driver_32bpp_horizontal_pixelmap_line_compressed_write(
        context, xstart, xend, y, info);
    drawn = GX_TRUE;
  }

  if (!synergy_specific_format && !drawn) {
    _gx_display_driver_32bpp_horizontal_pixelmap_line_draw(context, xstart,
                                                           xend, y, info);
    return;
  }
