...
/**************************************************************************/
static VOID
_gx_dave2d_simulation_display_driver_565rgb_rotated_pixelmap_compressed_write(
  ...
  if (context->gx_draw_context_display->gx_display_rotation_angle ==
    ...
    rotated_clip.gx_rectangle_top =
        (GX_VALUE)(context->gx_draw_context_canvas->gx_canvas_x_resolution -
                   clip->gx_rectangle_right - 1);
    rotated_clip.gx_rectangle_bottom =
        (GX_VALUE)(context->gx_draw_context_canvas->gx_canvas_x_resolution -
                   clip->gx_rectangle_left - 1);
    ypos = (context->gx_draw_context_canvas->gx_canvas_x_resolution - ypos -
            pixelmap->gx_pixelmap_width);
  } else {
    rotated_clip.gx_rectangle_left =
        (GX_VALUE)(context->gx_draw_context_canvas->gx_canvas_y_resolution -
                   clip->gx_rectangle_bottom - 1);
    rotated_clip.gx_rectangle_right =
        (GX_VALUE)(context->gx_draw_context_canvas->gx_canvas_y_resolution -
                   clip->gx_rectangle_top - 1);
    rotated_clip.gx_rectangle_top = clip->gx_rectangle_left;
    rotated_clip.gx_rectangle_bottom = clip->gx_rectangle_right;
    xpos = (context->gx_draw_context_canvas->gx_canvas_y_resolution - xpos -
          ...
          if (xval >= rotated_clip.gx_rectangle_left &&
            ...
            if (alpha == 0xff) {
              *put = pixel;
              ...
              _gx_display_driver_565rgb_pixel_blend(context, xval, yval, pixel,
                                                    alpha);
            }
          }
          put++;
        ...
        if (alpha_value) {
          ...
          while (count--) {
            if (xval >= rotated_clip.gx_rectangle_left &&
                xval <= rotated_clip.gx_rectangle_right) {
              if (alpha_value == 0xf0) {
                _gx_display_driver_16bpp_pixel_write(context, xval, yval,
                                                     pixel);
              } else {
                _gx_display_driver_565rgb_pixel_blend(context, xval, yval,
                                                      pixel, alpha_value);
              }
            }
            xval++;
...
/**************************************************************************/
static VOID
_gx_dave2d_simulation_display_driver_565rgb_4444bgra_rotated_pixelmap_compressed_alpha_write(
    GX_DRAW_CONTEXT *context, INT xpos, INT ypos, GX_PIXELMAP *pixelmap) {
  INT yval;
  INT xval;
  GX_CONST UCHAR *get;
...
/**************************************************************************/
static VOID
_gx_dave2d_simulation_display_driver_565rgb_32argb_rotated_pixelmap_raw_write(
    GX_DRAW_CONTEXT *context, INT xpos, INT ypos, GX_PIXELMAP *pixelmap) {
  INT skipcount;
  INT xval;
  INT yval;
  ...
  skipcount =
      (pixelmap->gx_pixelmap_height) * (rotated_clip.gx_rectangle_top - ypos);
  skipcount += (rotated_clip.gx_rectangle_left - xpos);

  getrow = (GX_COLOR *)(pixelmap->gx_pixelmap_data);
  getrow += skipcount;

  for (yval = rotated_clip.gx_rectangle_top;
       yval <= rotated_clip.gx_rectangle_bottom; yval++) {
    get = getrow;

    for (xval = rotated_clip.gx_rectangle_left;
         xval <= rotated_clip.gx_rectangle_right; xval++) {
      ...
      if (alpha == 0xff) {
        ...
        _gx_display_driver_565rgb_pixel_blend(context, xval, yval, color,
                                              alpha);
      }
      get++;
    }
...
VOID _gx_dave2d_simulation_display_driver_565rgb_rotated_pixelmap_blend(
  ...
  if (!synergy_specific_format && !drawn) {
    if (alpha == 0xff) {
      _gx_display_driver_565rgb_rotated_pixelmap_draw(context, xpos, ypos,
                                                      pixelmap);
    } else {
      _gx_display_driver_565rgb_rotated_pixelmap_blend(context, xpos, ypos,
                                                       pixelmap, alpha);
    }
  }
}
...
VOID _gx_dave2d_simulation_display_driver_565rgb_rotated_pixelmap_draw(
  ...
  _gx_dave2d_simulation_display_driver_565rgb_rotated_pixelmap_blend(
      context, xpos, ypos, pixelmap,
      context->gx_draw_context_brush.gx_brush_alpha);
}
