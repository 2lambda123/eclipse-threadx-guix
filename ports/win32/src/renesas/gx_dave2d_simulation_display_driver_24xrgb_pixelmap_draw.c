        ...
        if (has_alpha) {
          ...
          while (count--) {
            if (xval >= clip->gx_rectangle_left &&
              ...
              _gx_display_driver_24xrgb_pixel_blend(context, xval, yval, pixel,
                                                    alpha_value);
            }
            xval++;
          }
          ...
          while (count--) {
            if (xval >= clip->gx_rectangle_left &&
                xval <= clip->gx_rectangle_right) {
              if (brush_alpha != 0xff) {
                _gx_display_driver_24xrgb_pixel_blend(context, xval, yval,
                                                      pixel, brush_alpha);
              } else {
                _gx_display_driver_32bpp_pixel_write(context, xval, yval,
                                                     pixel);
              }
            }
            xval++;
...
/**************************************************************************/
static VOID
_gx_dave2d_simulation_display_driver_32bpp_4444argb_pixelmap_compressed_alpha_write(
    GX_DRAW_CONTEXT *context, INT xpos, INT ypos, GX_PIXELMAP *pixelmap) {
  INT yval;
  INT xval;
  GX_CONST UCHAR *get;
...
/**************************************************************************/
static VOID
_gx_dave2d_simulation_display_driver_32bpp_4444bgra_pixelmap_compressed_write(
    GX_DRAW_CONTEXT *context, INT xpos, INT ypos, GX_PIXELMAP *pixelmap) {
  INT yval;
  INT xval;
  GX_CONST UCHAR *get;
...
/**************************************************************************/
static VOID
_gx_dave2d_simulation_display_driver_24xrgb_565rgb_pixelmap_compressed_write(
    GX_DRAW_CONTEXT *context, INT xpos, INT ypos, GX_PIXELMAP *pixelmap) {
  INT yval;
  INT xval;
        ...
        if (brush_alpha == 0xff) {
          ...
          while (count--) {
            if (xval >= clip->gx_rectangle_left &&
              ...
              pixel = (USHORT)((*(get + 1)) << 8) | pixel;
              pixel_24 =
                  ASSEMBLECOLOR_32BPP((GX_COLOR)(REDVAL_16BPP(pixel) << 3),
                                      (GX_COLOR)(GREENVAL_16BPP(pixel) << 2),
                                      (GX_COLOR)(BLUEVAL_16BPP(pixel) << 3));
              _gx_display_driver_24xrgb_pixel_blend(context, xval, yval,
                                                    pixel_24, brush_alpha);
