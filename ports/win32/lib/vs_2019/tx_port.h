...
#define TX_TIMER_THREAD_STACK_SIZE                                             \
  400 /* Default timer thread stack size - Not used in Win32 port!  */
#endif
...
/* Define the Win32-specific initialization code that is expanded in the generic
 * source.  */
...
#define TX_THREAD_EXTENSION_0                                                  \
  HANDLE tx_thread_win32_thread_handle;                                        \
...
void _tx_win32_critical_section_obtain(
    TX_WIN32_CRITICAL_SECTION *critical_section);
void _tx_win32_critical_section_release(
    TX_WIN32_CRITICAL_SECTION *critical_section);
void _tx_win32_critical_section_release_all(
    TX_WIN32_CRITICAL_SECTION *critical_section);

/* Define post completion processing for tx_thread_delete, so that the Win32
 * thread resources are properly removed.  */
...
/* Define post completion processing for tx_thread_reset, so that the Win32
 * thread resources are properly removed.  */
...
CHAR _tx_version_id[] =
    ...
    "Win32/Visual Studio Version 6.4.0 *";
#else
